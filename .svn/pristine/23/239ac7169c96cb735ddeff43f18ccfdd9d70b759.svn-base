	  ...
	  // 回车事件
        ...
        $("#table").datagrid({
                ...
                onClickRow :function(rowIndex,rowData){
		            ...
		            $('#tt').tree({
		   			  //异步树 
		   		     url: bathPath+'/systemrolemenu/loadRoleMenuTree?roleId='+id+ "&random="+Math.random(),
			   		 ...
			   		 onBeforeExpand: function(node){
			   		    	if(node){
			   		    		$('#tt').tree('options').url = basePath + "/systemrolemenu/loadRoleMenuChildrenTree?roleId="+id+"&fID=" + node.id+ "&random="+Math.random();  
			   		    	}
			   		 }
			   		});
        ...
        save:function(){
            ...
            for (var i = 0; i < nodes.length; i++) {
                if (nodes[i].id != 0) {
                    ...
                    s += nodes[i].id+";"+nodes[i].attributes.roleMenuId;
                }
            }
            for (var i = 0; i < nodes1.length; i++) {
                if (nodes1[i].id != 0) {
                    ...
                    s += nodes1[i].id+";"+nodes1[i].attributes.roleMenuId;
                }
            }
            ...
            for (var i = 0; i < nodes2.length; i++) {
                if (nodes2[i].id != 0) {
                    ...
                    s1 += nodes2[i].id+";"+nodes2[i].attributes.roleMenuId;
                }
            }
            ...
            if(rows.length>0){
            	...
            	$.messager.alert('信息提示','请选择需要操作的角色','info');
            	return false;
            }
			$.ajax({            
                  type:"POST",   //post提交方式默认是get
                  url:bathPath+'/systemrolemenu/save',
                  data: { 
                	  ids:s,
                	  noids:s1,
                	  roleId:roleId
