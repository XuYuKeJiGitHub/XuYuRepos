...
$(function () {
        ...
        //下拉框值加载
		$("#parentId").combotree({
		        url:bathPath+'/systemmenu/loadMenuTree', //这是第一次加载树的url 加载根节点
		        onBeforeExpand:function(node){   //展开前获取数据
		        	 $("#parentId").combotree("tree").tree("options").url=bathPath+'/systemmenu/loadMenuChildrenTree?FID='+node.id; //这是点击根节点的时候发送请求去加载子节点
		        }
		 });
		$('#tt').tree({
			  //异步树 
		    url: bathPath+'/systemmenu/loadMenuTree',
		    onClick:function(node){
		    	 var opts = $("#table").datagrid("options");
	             opts.url = bathPath+'/systemmenu/findList';
		    	 $("#table").datagrid('load',{
		    		 parentId:node.id
                 })
		    },
		    onBeforeExpand: function(node){
		    	if(node){
		    		$('#tt').tree('options').url = basePath + "/systemmenu/loadMenuChildrenTree?FID=" + node.id;  
		    	}
		    } 
		});
        ...
        function find(pageNumber, pageSize)
        {
                ...
                $.ajax({
                    type : "POST",
                    dataType : "json",
                    url:bathPath+'/systemmenu/findList',
                    data : {
